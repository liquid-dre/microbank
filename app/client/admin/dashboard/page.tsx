...
// Sortable keys
type SortKey = "name" | "email" | "isBlacklisted" | "isAdmin";
...
export default function AdminDashboard() {
	...
	const toggleBlacklist = async (id: string) => {
		try {
			...
			setClients((prev) =>
				prev.map((c) =>
					c.id === id ? { ...c, isBlacklisted: updated.isBlacklisted } : c
				)
			);
		} catch {
	...
	const toggleAdmin = async (id: string) => {
		try {
			...
			setClients((prev) =>
				prev.map((c) => (c.id === id ? { ...c, isAdmin: updated.isAdmin } : c))
			);
		} catch {
			...
			<div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
				{[
					...
					},
					{
						...
						color: "#DC2626",
						progress: totalCount
							? Math.round((blacklistedCount / totalCount) * 100)
							: 0,
					},
					{
						label: "Active",
			...
			<div className="overflow-x-auto bg-white rounded-lg shadow">
				<table className="w-full table-auto border-collapse">
					<thead className="bg-gray-100">
						<tr>
							{["name", "email", "isBlacklisted", "isAdmin"].map((col) => (
								<th
									key={col}
									...
									>
										...
										<td className="p-4">
											<Switch
												...
												onCheckedChange={() => toggleAdmin(client.id)}
											/>
										</td>
									</motion.tr>
								))
